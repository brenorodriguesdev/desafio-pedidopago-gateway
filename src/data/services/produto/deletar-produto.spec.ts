...
const makeSut = (): SutTypes => {
    ...
    const sut = new DeletarProdutoService(grpcClient)
    return {
        grpcClient,
        sut
    }
...
describe('DeletarProduto Service', () => {
    test('Garantir que call seja chamado com os valores corretos', async () => {
        ...
        await sut.deletar(1)
        expect(callSpy).toHaveBeenCalledWith('produto.proto', process.env.GRPC_CONNECTION_PRODUTO, 'ProdutoService', 'deletar', { id: 1 })
    })
