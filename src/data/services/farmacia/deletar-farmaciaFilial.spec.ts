...
const makeSut = (): SutTypes => {
    ...
    const sut = new DeletarFarmaciaFilialService(grpcClient)
    return {
        grpcClient,
        sut
    }
...
describe('DeletarFarmaciaFilial Service', () => {
    test('Garantir que call seja chamado com os valores corretos', async () => {
        ...
        await sut.deletar(1)
        expect(callSpy).toHaveBeenCalledWith('farmacia.proto', process.env.GRPC_CONNECTION_FARMACIA, 'FarmaciaService', 'deletarFilial', { id: 1 })
    })
